  ...
  return (
    <section id="test" className="sec-pad bg-white overflow-hidden">
      ...
      <p className="text-center mt-[clamp(2.2rem,4vw,3rem)] text-[0.85rem] text-muted max-w-[680px] mx-auto px-6">
        <span className="text-lav">✦</span> Testing depth varies by program tier. Metabolic precision and genetic &amp; tolerance markers are reviewed by AHPRA-registered practitioners.
      </p>
    </section>
  );
}
...
function ProgressTracked() {
  ...
  const color = { optimal: "text-emerald-600 bg-emerald-50", monitor: "text-amber-600 bg-amber-50", action: "text-lav bg-lavsoft" };
  return (
  ...
  if (idx === 0) return (
    <div>
      <div className="flex items-center gap-3 px-1.5 pb-4 pt-1"><span className="text-[0.72rem] text-muted whitespace-nowrap">Section · 2/4</span>
        <span className="flex gap-1.5 flex-1">{[1, 1, 0, 0].map((on, i) => <i key={i} className={"h-[5px] flex-1 rounded-full " + (on ? "bg-gradient-to-r from-blue to-[#7aa5ff]" : "bg-line2")}></i>)}</span>
      </div>
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="rounded-2xl border border-line bg-white p-[1.1rem]">
        ...
        {[["Oral therapy", "Daily · review in 4 weeks"], ["GLP-1 (titrated)", "Weekly · CGM-monitored"], ["Nutrient support", "Daily · personalised"]].map(([n, p]) => (
          <div key={n} className="flex items-center gap-2.5 py-2 border-b border-line last:border-0"><span className="w-[30px] h-9 rounded-md bg-gradient-to-b from-[#eceaf6] to-[#d9d4ee] shrink-0"></span><div><div className="text-[0.82rem] font-semibold">{n}</div><div className="text-[0.66rem] text-muted">{p}</div></div></div>
        ))}
      </div>
