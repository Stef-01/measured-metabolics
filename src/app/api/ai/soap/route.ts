...
export async function POST(req: NextRequest) {
  ...
  try {
    const raw = await runStructured({
      schemaName: "soap-draft",
      variables: { transcript, context },
    });
    const safe = applySafety("soap-draft", raw);
    // Return flattened so the client receives {subjective, objective, ...} directly.
