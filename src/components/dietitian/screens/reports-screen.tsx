...
import { FileText, Send, ArrowRight, TrendingDown, TrendingUp, Minus } from "lucide-react";
...
function StatChip({
  ...
  const tone: Record<StatStatus, string> = {
    ...
    watch: "bg-[var(--measured-clinical-amber)]/15 text-[var(--measured-clinical-amber)]",
    high: "bg-[var(--measured-evaluate)]/10 text-[var(--measured-evaluate)]",
  };

  return (
    <div className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold", tone[status])}>
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <ul className="mt-6 grid gap-3 md:grid-cols-2">
        {myPatients.map((p) => {
          ...
          const hbA1cStatus: StatStatus = p.hbA1cPct < 7 ? "good" : p.hbA1cPct < 8 ? "watch" : "high";
          const tirStatus: StatStatus = p.timeInRangePct >= 70 ? "good" : p.timeInRangePct >= 55 ? "watch" : "high";
          const wtStatus: StatStatus = p.weightDeltaKg < 0 ? "good" : p.weightDeltaKg < 1 ? "watch" : "high";
          return (
            <li key={p.id}>
              <Link
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <StatChip label="HbA1c" value={`${p.hbA1cPct}%`} status={hbA1cStatus} lowerBetter />
                      <StatChip label="TIR" value={`${p.timeInRangePct}%`} status={tirStatus} />
                      <StatChip
                        label="Wt Δ"
